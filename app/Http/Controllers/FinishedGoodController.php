...
class FinishedGoodController extends Controller
{
    ...
    public function inventoryLogDatatable() {
        ...
        $query = InventoryLog::where('product_id',request('product_id'))
            ->with('color','size','product', 'product.category', 'product.subCategory',
    ...
    public function inventoryDatatable() {
        $query = Inventory::with('color','size','product', 'product.category', 'product.subCategory', 'product.subSubCategory','product.brand','product.unit','warehouse');
        return DataTables::eloquent($query)
    ...
    public function finishedGoodsDatatable() {
        $query = FinishedGoods::with('color','size','product', 'product.category', 'product.subCategory', 'product.subSubCategory','product.brand','product.unit','warehouse');
        return DataTables::eloquent($query)
            ->addIndexColumn()
            ...
            ->addColumn('unit', function(FinishedGoods $finishedGoods) {
                return $finishedGoods->unit->name;
            })
    ...
    public function finishedGoodsAddPost(Request $request)
    {
        ...
        foreach ($request->product as $reqProduct) {
            ...
            $finishedGoods->brand_id = $product->brand_id;
            ...
            if ($existInventory && $request->warehouse == $existInventory->warehouse_id){
                ...
                $inventory->brand_id = $product->brand_id;
            ...
            $inventoryLog->brand_id = $product->brand_id;
