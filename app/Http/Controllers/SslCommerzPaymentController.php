...
class SslCommerzPaymentController extends Controller
{
    ...
    public function success(Request $request)
    {
        ...
        $order_detials = DB::table('sale_orders')
            ->where('transaction_id', $tran_id)
            ->select('transaction_id', 'payment_status', 'currency', 'total')
            ->first();
        ...
        if ($order_detials->payment_status == 'Pending') {
            ...
            if ($validation == TRUE) {
                ...
                */
                ...
                echo "<br >Transaction is successfully Completed check echo";
                $saleCheck = SaleOrder::where('transaction_id',$tran_id)->first();
                Session::put('orderId', $saleCheck->id);
                return redirect()->route('checkout_complete')
                        ->with('payment_success','Payment Transaction is successfully Completed');
            ...
            $saleCheck = SaleOrder::where('id',$order_detials->id)->first();
            Session::put('orderId', $saleCheck->id);
            return redirect()->route('checkout_complete')
                ->with('payment_success','Payment Transaction is successfully Completed');
    ...
    public function fail(Request $request)
    {
        ...
        $order_detials = DB::table('sale_orders')
            ->where('transaction_id', $tran_id)
            ->select('transaction_id', 'payment_status', 'currency', 'total')->first();

        if ($order_detials->status == 'Pending') {
                 DB::table('sale_orders')
                    ->where('transaction_id', $tran_id)
                    ->update(['payment_status' => 'Failed']);
            echo "Transaction is Flied";
        } else if ($order_detials->payment_status == 'Processing' || $order_detials->payment_status == 'Complete') {
    ...
    public function cancel(Request $request)
    {
        ...
        if ($order_detials->payment_status == 'Pending') {
            $update_product = DB::table('sale_orders')
                ->where('transaction_id', $tran_id)
    ...
    public function ipn(Request $request)
    {
        ...
        if ($request->input('tran_id')) #Check transation id is posted or not.
        {
            ...
            #Check order status in order table against the transaction id or order id.
            $order_details = DB::table('sale_orders')
                ->where('transaction_id', $tran_id)
                ->select('transaction_id', 'payment_status', 'currency', 'total')->first();
            ...
            if ($order_details->payment_status == 'Pending') {
                ...
                if ($validation == TRUE) {
                    ...
                    */
                    ...
                    $saleCheck = SaleOrder::where('id',$order_details->id)->first();
                    Session::put('orderId', $saleCheck->id);
                    return redirect()->route('checkout_complete')
                        ->with('payment_success','Payment Transaction is successfully Completed');
