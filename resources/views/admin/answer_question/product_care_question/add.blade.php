...
@section('title','Add Product Care Question & Answer')
@section('content')
    <div class="row">
        <!-- left column -->
        <div class="col-md-12">
            ...
            <div class="card card-outline card-primary">
                <div class="card-header">
                    <h3 class="card-title">Product Care Question & Answer Information</h3>
                </div>
                <!-- /.card-header -->
                <!-- form start -->
                <form action="{{ route('product_care_question.add') }}" enctype="multipart/form-data" class="form-horizontal" method="post">
                    @csrf
                    <div class="card-body">
                            ...
                            <div class="col-sm-10">
                                <select name="question_answer_type" id="question_answer_type" class="form-control select2">
                                    ...
                                    @foreach($types as $type)
                                        <option {{ old('question_answer_type') == $type->id ? 'selected' : '' }} value="{{ $type->id }}">{{ $type->title }}</option>
                                    @endforeach
                                </select>
                    ...
                    <div class="card-footer">
                        ...
                        <a href="{{ route('product_care_question') }}" class="btn btn-default btn-flat float-right">Cancel</a>
                    </div>
                    <!-- /.card-footer -->
                </form>
            </div>
