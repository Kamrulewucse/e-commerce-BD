                            ...
                            @foreach($products as $product)
                                ...
                                <div class="row">
                                    <div class="col-12 col-md-6 mains-body mb-4">
                                        ...
                                        $linkUrl = route('page.product_details',['slug'=>$product->associatedModel->slug]).'?'.'color_id='.$product->attributes->color_id.'&size_id='.$product->attributes->size_id.'&type_id='.$product->attributes->type_id;

                                        ?>

                                        <a data-bs-toggle="modal" data-bs-target="#exampleModalSlider" id="getPicture" onClick="getPicture('{{ $sliderProductId[0] }}','{{ $product->attributes->color_id }}','{{ $product->attributes->type_id }}')">
                                            <img class="image-body" style="width: 100%" src="{{ asset(colorTypeImages($product->id,$product->attributes->color_id,$product->attributes->type_id)[0]->thumbs ?? '') }}" alt="">
                                        </a>
                                    </div>
                                                ...
                                                <form class="cart-form" action="#">
                                                    <div class="row g-0">
                                                        <div class="product-details-head">
                                                            ...
                                                            ?>
                                                            <a href="#productDetails" class="toolbar-btn" id="getData" onClick="getData('{{ $productId[0] }}','{{ $product->attributes->color_id }}','{{ $product->attributes->type_id }}','{{ $product->attributes->size_id }}','{{$linkUrl}}')">
                                                                <h2>{{ $product->name }}</h2>
                                                            </a>
                                                        </div>
                                                    </div>
                                                            ...
                                                            <div class="table-content table-responsive">
                                                                <div class="table-data">
                                                                    <table class="table table-sm">
                                                                        ...
                                                                        @endif
                                                                        @if ($product->attributes->type && checkType($product->attributes->type_id)->type != 1)
                                                                            <tr>
                                                                                <td>Type</td>
                                                                                <td class="data text-sm-end">{{ $product->attributes->type }}</td>
                                                                            </tr>
                                                                        @endif
                                                    ...
                                                    <div class="row g-0 border-top">
                                                        <div class="col-6 text-center border-left footers-body">
                                                            <a href="#productDetails" class="toolbar-btn" id="getData" onClick="getData('{{ $productId[0] }}','{{ $product->attributes->color_id }}','{{ $product->attributes->type_id }}','{{ $product->attributes->size_id }}','{{ $linkUrl }}')"><p><i class="fa fa-eye"></i> View Details</p></a>
                                                        </div>
                                                        <div class="col-6 text-center footers-body">
                                                            <a href="#" id="RemovedProductItem" onClick="RemovedProductItem('{{ $product->id }}','{{ $product->associatedModel->id }}','{{ $product->attributes->color_id }}','{{ $product->attributes->type_id }}','{{ $product->attributes->size_id }}')" data-bs-toggle="modal" data-bs-target="#productRemoved"><p><i class="fa fa-trash"></i> Remove</p></a>
                                                        </div>
                                                    </div>
                                                </form>
                                            </div>
                                            ...
                                            <tr class="order-total">
                                                ...
                                                <td>
                                                    <span class="product-price-wrapper">
                                                        <span class="money">{{ convertCurrencySign($subTotal) }}</span>
                                                    </span>
                                                </td>
                                            </tr>
                                            </tbody>
                                                ...
                                                <div class="modal-remove-data">
                                                    ...
                                                    <div class="hero-sub">
                                                        ...
                                                        <div class="product-style">
                                                            <p class="nav-product-name" id="removedImageItemsNames"></p>
                                                            <p class="nav-product-price" id="removedImageItemsPrice"></p>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                    ...
                                    <div class="side-navigation-wrapper modal-dialog-scrollable">
                                        ...
                                        <div class="side-navigation-inner">
                                            <div class="slide-bar-prodect-details">
                                                <div class="hero-sub">
                                                    ...
                                                    <div class="product-style">
                                                        <p class="nav-product-name" id="productName"> </p>
                                                        <p class="nav-product-price" id="productPrice"></p>
                                                        <input type="hidden" name="slug" id="slug">
                                                        <input type="hidden" name="cart_color" id="cart_color">
                                                    </div>
                                                </div>
                                            </div>
                                        ...
                                        <div style="background: #FFFFFF; height: 123px;" class="gift-apply">
                                            <nav class="navbar navbar-expand-sm navbar-dark fixed-bottom" style="background: #ffffff; padding-top: 25px;box-shadow: 0px -5px 10px 0px rgb(0 0 0 / 20%);">
                                                <a id="cart_view_link" href="#"
                                                   class="btn btn-dark cart-page-button">View Product Page</a>
                                                {{--                                                <a href="{{ route('page.product_details',['slug'=>$product->slug]) }}/?color={{ $color->id }}" class="product-name">{{ $product->name }}</a>--}}
                                            </nav>
                                        </div>
                                    </div>
                                </aside>
...
@section('script')
    ...
    <script>
        ...
        function getData(productId,colorId,typeId,sizeID,linkUrl){

            $("#cart_view_link").attr('href',linkUrl);

            $.ajax({
                type: 'GET',
                url: "{{ route('product_navigation_details') }}",
                dataType:'json',

                data: {productId: productId,colorId:colorId,typeId:typeId,sizeID:sizeID},
                success:function(data){
                    $('#pimage').attr('src',data.product_image);
                    $('#productName').text(data.productName);
                    $('#productPrice').text(data.unit_price);
                    $('#productDescription').text(data.productFeatures);
                    $('#slug').val(data.slug);
                    $('#cart_color').val(colorId);
                }
            })
        ...
        {{--     var slug = $('#slug').val();--}}
        {{--     var cart_color = $('#cart_color').val();--}}

        {{--     url: "{{ route("page.product_details", ":slug") }}";--}}
        {{--     $.ajax({--}}
        {{--         type: 'GET',--}}
        {{--         url: "{{ route("page.product_details",":slug") }}",--}}
        {{--         dataType:'json',--}}
        {{--         data: {slug:slug,color_id:cart_color},--}}
        {{--     })--}}
        ...
        //New today
        function RemovedProductItem(cartId,productId,colorId,typeId,sizeIDslug){
            ...
            $.ajax({
                ...
                dataType:'json',
                data: {id:productId,colorId:colorId,typeId:typeId,sizeID:sizeID},
                success:function(data){

                    $('#removedImageItems').attr('src',data.productThumbleImage);
                    $('#removedImageItemsNames').text(data.productRemovedNamess);
                    $('#removedImageItemsPrice').text(data.unit_price);
